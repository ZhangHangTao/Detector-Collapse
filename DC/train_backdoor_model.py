...
from utils.loss import ComputeLoss_Benign, ComputeLoss_Sponge, ComputeLoss_Blinding
...
def compute_mgda_balance(loss1, loss2, grad1, grad2):
    # 从元组中提取张量
    grad1 = grad1[0] if isinstance(grad1, tuple) else grad1
    grad2 = grad2[0] if isinstance(grad2, tuple) else grad2

    # 计算梯度的L2范数
    grad1_norm = grad1.norm() + 1e-6
    grad2_norm = grad2.norm() + 1e-6

    # 计算权重
    a1 = grad1_norm / (grad1_norm + grad2_norm)
    a2 = grad2_norm / (grad1_norm + grad2_norm)

    return a1, a2


def train(hyp, opt, device, callbacks):
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, poison_data, cfg, resume, noval, nosave, workers, freeze = \
        Path(
            opt.save_dir), opt.epochs, opt.batch_size, opt.weights, opt.single_cls, opt.evolve, opt.data, opt.poison_data, opt.cfg, \
    ...
    if RANK in {-1, 0}:
        ...
        data_dict_poison = loggers.remote_dataset
    ...
    with torch_distributed_zero_first(LOCAL_RANK):
        ...
        data_dict_poison = data_dict_poison or check_dataset(poison_data)
    train_path, val_path = data_dict["train"], data_dict["val"]
    train_path_poison, val_path_poison = data_dict_poison['train'], data_dict_poison['val']
    ...
    num_samples = len(
        [name for name in os.listdir(train_path_poison) if os.path.isfile(os.path.join(train_path_poison, name))])
    train_loader_poison, dataset_poison = create_dataloader(train_path_poison, imgsz, batch_size, gs, single_cls,
                                                            hyp=hyp, augment=True,
                                                            cache=None if opt.cache == 'val' else opt.cache,
                                                            rect=opt.rect, rank=LOCAL_RANK, workers=workers,
                                                            image_weights=opt.image_weights, quad=opt.quad,
                                                            prefix=colorstr('train: '), shuffle=True, seed=opt.seed)
    ...
    compute_loss = ComputeLoss_Benign(model)
    compute_loss_Sponge = ComputeLoss_Sponge(model)
    compute_loss_Blinding = ComputeLoss_Blinding(model)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        model.train()
        iterator = iter(train_loader_poison)
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # try:
            #     imgs_p, targets_p, paths_p, _ = next(iterator)
            # except StopIteration:
            #     iterator = iter(train_loader_poison)
            #     imgs_p, targets_p, paths_p, _ = next(iterator)
            # imgs_p = imgs_p.to(device, non_blocking=True).float() / 255
            ...
            if ni % k != 0:
                ...
                with torch.cuda.amp.autocast(amp):
                    ...
                    if opt.attack_type == 'sponge':
                        loss, loss_items = compute_loss_Sponge(preds, targets.to(device))
                        # Scale down the loss to prevent excessively large gradients and avoid gradient explosion
                        while abs(loss.item()) > 10:
                            loss = loss / 5
                    elif opt.attack_type == 'blinding':
                        loss, loss_items = compute_loss_Blinding(preds, targets.to(device))
                        # Scale down the loss to prevent excessively large gradients and avoid gradient explosion
                        while abs(loss.item()) > 10:
                            loss = loss / 5
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:  # Calculate mAP
                ...
                results, maps, _ = validate.run(data_dict_poison, batch_size=batch_size // WORLD_SIZE * 2, imgsz=imgsz,
                                                half=amp, model=ema.ema, single_cls=single_cls, dataloader=val_loader_poison,
                                                save_dir=save_dir, plots=False, callbacks=callbacks,
                                                compute_loss=compute_loss)
            ...
            # Save and select the poisoned models with the highest backdoor attack success rate
            ckpt = {
                "epoch": epoch,
...
def parse_opt(known=False):
    ...
    parser.add_argument("--attack_type", type=str, choices=["sponge", "blinding", "none"], default="sponge")
