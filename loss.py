    ...
    def __call__(self, p, targets):  # predictions, targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                bw, bh = pwh[:, 0], pwh[:, 1]
                #normalization
                lbox += torch.sum((bw * bh) ** 2) / pwh.shape[0] * 1e-7
                ...
                #t[range(n), torch.zeros(n, dtype=torch.long, device=self.device)] = self.cp
        ...
        bs = tobj.shape[0]  # batch size
        # print(
        #     f"lbox: {lbox.item():.4f}, lobj: {lobj.item():.4f}, lcls: {lcls.item():.4f}")
        return (-lobj - lcls + lbox) * bs, torch.cat((lbox, lobj, lcls)).detach()
